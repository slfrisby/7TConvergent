...
setwd("C:/Users/slfri/ownCloud/7T_WISC_MVPA/derivatives/classification/SOSLASSO/visualize")
...
#  .x$data <- file.path(data_path, .y$data)
  ...
  .x$alpha <- list(.y$alpha)
  .x$lambda <- list(.y$lambda)
  .x$SearchWithHyperband <- FALSE
  .x$HYPERBAND <- NULL
  .x$BRACKETS <- NULL
  .x$EXPAND <- list(c('alpha','lambda'))
...
stub_perm <- stub_final %>%
  map(~{
    .x$EXPAND <- list(c('alpha','lambda'),'RandomSeed')
