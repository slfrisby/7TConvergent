...
setwd("C:/Users/slfri/ownCloud/7T_WISC_MVPA/derivatives/classification/SOSLASSO/performance")
...
best_df <- tune_df %>% map(., ~{
  .x %>%
    group_by(finalholdout, alpha, lambda)  %>%
    summarize(
      err1 = mean(err1),
      err2 = mean(err2),
      nzvox = mean(nzvox)
...
stub_final <- map2(stub_tune, best_df, ~{
  ...
  .x$HYPERBAND <- NULL
  .x$BRACKETS <- NULL
  .x$EXPAND <- list(c('cvholdout','alpha','lambda'))
  .x$URLS <- list(c('metadata'))
  return(.x)
})
...
stub_perm <- stub_final %>%
    map(~{
        .x$EXPAND <- list(c('cvholdout','alpha','lambda'),'RandomSeed')
        .x$URLS <- c('metadata','PermutationIndex')
